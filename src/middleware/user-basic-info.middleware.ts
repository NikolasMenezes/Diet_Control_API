...
export function verifyUserBasicInfoFields(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  const validationRules = [
    ...
    },
    {
      field: 'height',
      required: true,
      message: "The field 'height' is required",
    },
    {
      field: 'weight',
      required: true,
      message: "The field 'weight' is required",
    },
  ];
  ...
  for (const rule of validationRules) {
    if (rule.options && !rule.options.includes(req.body[rule.field])) {
      errors.push({
        field: rule.field,
        message: `${rule.field} was incorrectly informed`,
      });
    }
